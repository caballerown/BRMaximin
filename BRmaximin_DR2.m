%% Behaviorally Robust Maximin -- DR2
%%
% This is a numerical approximation of the program DR2 (Caballero & Lunday) using a
% over the interval U_tau (i.e., [tau_LB, tau_UB]) of spacing tau_inc. With a
% fine enough grid interval, the solution converges to the optimal DR2 soln

% Accepts  inputs: (1) The normal form game (i.e., payoffarray), 
% (2,3) the lower and upper range of tau's uncertainty set, tau_LB & tau_UB 
% (4) the considered approximation spacing b/w elements, tau_inc 
%(5) mean of the unknown dist, c1 (6,7,8) the density of the distribution 
% (c2) within some subset of the uncertainty set (i.e., c3 and c4), 
%(9) a maximum k value to be considered, max_k & (10) which agent's role we 
% are taking (i.e, who are we in the game: agent).
...
% tau_LB, tau_UB and tau_inc are double values (both greater than 0)
...
function [x] = BRmaximin_DR2(payoffarray, tau_LB, tau_UB, tau_inc, c1,c2,c3,c4, max_k, agent)
