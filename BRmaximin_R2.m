%% Behaviorally Robust Maximin -- R2
%%
...
% (2004). These values are used to approximate the LP described as R2 by Caballero, 
% Lunday and Uberbased on a grid over the interval tau (i.e., [tau_LB, tau_UB])
% of spacing tau_inc. With a fine enough grid interval, 
% the solution converges to the optimal R2 sol'n
...
% (5) a maximum k value to be considered, max_k, and (6) which agent's role 
% we are taking (i.e, who are we in the game -- agent).
...
function [x] = BRmaximin_R2(payoffarray, tau_LB, tau_UB, tau_inc, max_k, agent)
